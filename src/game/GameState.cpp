...
void connectShipManagerField(ShipManager &shipManager, Field &field)
{
	for (int i = 0; i < shipManager.GetNumberBattleships(); i++)
	{
		...
		std::cout << "x " << position.x << " y " << position.y << " d " << direction << "\n"
				  << field << '\n';
		field.SetBattleship(position.x, position.y, ship, direction);
	}
}
FileWrapper &operator>>(FileWrapper &fileWrapper, GameState &state)
{
